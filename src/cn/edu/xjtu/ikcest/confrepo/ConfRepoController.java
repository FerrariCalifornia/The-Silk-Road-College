 ...
 * @Title: ConfRepoController.java  
 * @Description: conf_repo Controller类 
 * @date 2016-07-30 
 * @since V1.0.0 
 *
...
/**  
 * @ClassName: ConfRepoController  
 * @Description: conf_repoController    
 * @since V1.0.0  
 */
@RequiresAuthentication
@RequiresPermissions({ "confRepo" })
@MappingTo("/confRepo")
public class ConfRepoController extends SqlBuilderBaseCrudController<java.lang.String,ConfRepo> {
    
    private static final Log LOG = Log.getLog(ConfRepoController.class);

    private static final ConfRepoTable CONF_REPO = ConfRepoTable.me();
    
    public ConfRepoController(){
        //设定是否支持自动添加操作日志
    ...
    @Override
    protected ConfRepo getDaoModel() {
        return ConfRepo.DAO;
    }
    ...
    @Override
    public void index() {
        this.render("/cn/edu/xjtu/ikcest/confrepo/tpl/index.tpl");
    }
    ...
    public void uploadFile(){
        ...
        DataReader reader = new ExcelDataReader("conf_repo", file.getFile(), ConfRepo.DAO.getTitleMap());
        DataTable dataTable = reader.read();
        int importNum = 0;
        int existsNum = 0;
        if (dataTable.getSize() > 0) {
            for(Map<String, Object> row : dataTable.getRows()){
                ConfRepo item = new ConfRepo();
    ...
    @Override
    protected AjaxReturn beforeCreate(ConfRepo m) {
