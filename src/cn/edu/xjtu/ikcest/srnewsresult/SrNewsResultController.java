 ...
 * @Title: SrNewsResultController.java  
 * @Description: sr_news_result Controller类 
 * @date 2016-07-30 
 * @since V1.0.0 
 *
...
/**  
 * @ClassName: SrNewsResultController  
 * @Description: sr_news_resultController    
 * @since V1.0.0  
 */
@RequiresAuthentication
@RequiresPermissions({ "srNewsResult" })
@MappingTo("/srNewsResult")
public class SrNewsResultController extends SqlBuilderBaseCrudController<java.lang.String,SrNewsResult> {
    
    private static final Log LOG = Log.getLog(SrNewsResultController.class);

    private static final SrNewsResultTable SR_NEWS_RESULT = SrNewsResultTable.me();
    
    public SrNewsResultController(){
        //设定是否支持自动添加操作日志
    ...
    @Override
    protected SrNewsResult getDaoModel() {
        return SrNewsResult.DAO;
    }
    ...
    @Override
    public void index() {
        this.render("/cn/edu/xjtu/ikcest/srnewsresult/tpl/index.tpl");
    }
    ...
    protected SelectBuilder beforeList(SelectBuilder sql) {
        ...
        if(StrKit.notBlank(keyword)){
            //sql.and(SR_NEWS_RESULT.NAME.like("%"+keyword+"%"));
        }
        return sql;
    }

    public void downloadTemplate(){
        this.renderFile("sr_news_result.xls");
    }
    ...
    public void uploadFile(){
        ...
        DataReader reader = new ExcelDataReader("sr_news_result", file.getFile(), SrNewsResult.DAO.getTitleMap());
        DataTable dataTable = reader.read();
        int importNum = 0;
        int existsNum = 0;
        if (dataTable.getSize() > 0) {
            for(Map<String, Object> row : dataTable.getRows()){
                SrNewsResult item = new SrNewsResult();
